...
// here http://weatherband.itp.io:3000/data/all?macAddress=A4:CF:12:8A:C8:24
let starting_id = 9003;

function setup() {
  createCanvas(5000, 900);
  
  // Request the data from the weather band database
  let url = "https://cors-anywhere.herokuapp.com/http://weatherband.itp.io:3000/data/date?macAddress=A4:CF:12:8A:C8:24&from='2020-12-22'&to='2020-12-25'";
...
function drawGraphs() { 
  ...
  text("Weather data from " + recorded_at[0] + "(GMT) and onwards", 30, 60);
  text("Data collected with a DIY weather station in East Village, NY", 30, 80);
  text("by ITP Weather Band", 30, 100);
  text("rainin", 30, 200);
  for (i = 0; i < recorded_at.length; i++) {
    let mapped = map(rainin[i], 0, 1, 250, 100);
  ...
  for (i = 0; i < recorded_at.length; i++) {
    let mapped = map(temperature[i], 0, 20, 550, 500);
    ellipse(130+i, mapped, 0.5, 0.5);        
  }
  ...
  for (i = 0; i < recorded_at.length; i++) {
    let mapped = map(uvindex[i], 0, 1, 750, 700);
    ellipse(130+i, mapped, 0.5, 0.5);        
  }
  text("humidity", 30, 800);
  for (i = 0; i < recorded_at.length; i++) {
    let mapped = map(humidity[i], 50, 150, 850, 800);
    ellipse(130+i, mapped, 0.5, 0.5);        
  }
}
